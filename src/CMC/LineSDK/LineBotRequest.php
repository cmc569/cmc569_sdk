...
class LineBotRequest {
    ...
    public function send($data=array(), $tracking=array()) {
        ...
        foreach ($data['messages'] as $k => $v) {
            ...
            switch($webhook_type) {
                case 'text' :
                        $msg = $v[$webhook_type] ;
                        if (empty($v['sender'])) $res[] = $this->textConstruct($msg) ;
                        else $res[] = array_merge($this->textConstruct($msg), ['sender' => $v['sender']]);

                        break ;

                case 'sticker' :
                        $arr = $v[$webhook_type] ;
                        if (empty($v['sender'])) $res[] = $this->stickerConstruct($arr) ;
                        else $res[] = array_merge($this->stickerConstruct($arr), ['sender' => $v['sender']]);
                        unset($arr) ;
                ...
                case 'image' :
                        $arr = $v[$webhook_type] ;
                        if (empty($v['sender'])) $res[] = $this->imageConstruct($arr) ;
                        else $res[] = array_merge($this->imageConstruct($arr), ['sender' => $v['sender']]);
                        unset($arr) ;
                ...
                case 'video' :
                        $arr = $v[$webhook_type] ;
                        if (empty($v['sender'])) $res[] = $this->videoConstruct($arr) ;
                        else $res[] = array_merge($this->videoConstruct($arr), ['sender' => $v['sender']]);
                        unset($arr) ;
                ...
                case 'audio' :
                        $arr = $v[$webhook_type] ;
                        if (empty($v['sender'])) $res[] = $this->audioConstruct($arr) ;
                        else $res[] = array_merge($this->audioConstruct($arr), ['sender' => $v['sender']]);
                        unset($arr) ;
                ...
                case 'location' :
                        $arr = $v[$webhook_type] ;
                        if (empty($v['sender'])) $res[] = $this->locationConstruct($arr) ;
                        else $res[] = array_merge($this->locationConstruct($arr), ['sender' => $v['sender']]);
                        unset($arr) ;
                ...
                case 'imagemap' :
                        $arr = $v[$webhook_type] ;
                        if (empty($v['sender'])) $res[] = $this->imagemapConstruct($arr) ;
                        else $res[] = array_merge($this->imagemapConstruct($arr), ['sender' => $v['sender']]);
                        unset($arr) ;
                ...
                case 'confirm' :
                        $arr = $v[$webhook_type] ;
                        if (empty($v['sender'])) $res[] = $this->confirmConstruct($arr) ;
                        else $res[] = array_merge($this->confirmConstruct($arr), ['sender' => $v['sender']]);
                        unset($arr) ;
                ...
                case 'button' :
                        $arr = $v[$webhook_type] ;
                        if (empty($v['sender'])) $res[] = $this->buttonConstruct($arr) ;
                        else $res[] = array_merge($this->buttonConstruct($arr), ['sender' => $v['sender']]);
                        unset($arr) ;
                ...
                case 'carousel' :
                        ...
                        if (empty($v['sender'])) $res[] = $this->carouselConstruct($arr) ;
                        else $res[] = array_merge($this->carouselConstruct($arr), ['sender' => $v['sender']]);
                        unset($arr) ;
                ...
                case 'imagecarousel' :
                        $arr = $v[$webhook_type] ;
                        if (empty($v['sender'])) $res[] = $this->imageCarouselConstruct($arr) ;
                        else $res[] = array_merge($this->imageCarouselConstruct($arr), ['sender' => $v['sender']]);
                        unset($arr) ;
                ...
                case 'flex' :
                        $arr = $v[$webhook_type] ;
                        if (empty($v['sender'])) $res[] = $this->flex($arr) ;
                        else $res[] = array_merge($this->flex($arr), ['sender' => $v['sender']]);
                        unset($arr) ;
                ...
                case 'flexJson' :
                        $arr = $v[$webhook_type] ;
                        if (empty($v['sender'])) $res[] = $this->flexJson($arr) ;
                        else $res[] = array_merge($this->flexJson($arr), ['sender' => $v['sender']]);
                        unset($arr) ;
                ...
                case 'quickReply' :
                        $arr = $v[$webhook_type] ;
                        if (empty($v['sender'])) $res[] = $this->quickReply($arr) ;
                        else $res[] = array_merge($this->quickReply($arr), ['sender' => $v['sender']]);
                        unset($arr) ;
    ...
    private function textConstruct($txt) {
        ...
        if (empty($txt) || (mb_strlen($txt, $txt_type) > $limit)) return false ;
        else {
    ...
    private function stickerConstruct($arr) {
        if (empty($arr['packageId']) || empty($arr['stickerId'])) return false ;
        else {
    ...
    private function imageConstruct($arr) {
        if (empty($arr['imageUrl']) || empty($arr['previewUrl'])) return false ;
        else {
    ...
    private function videoConstruct($arr) {
        if (empty($arr['videoUrl']) || empty($arr['previewUrl'])) return false ;
        else {
            $template = array('type' => 'video', 'originalContentUrl' => $arr['videoUrl'], 'previewImageUrl' => $arr['previewUrl']) ;
            if (isset($arr['trackingId'])) $template['trackingId'] = $arr['trackingId'];
            return $template ;
        }
    }
    ##
    ...
    private function locationConstruct($arr) {
        if (empty($arr['title']) || empty($arr['address']) || empty($arr['latitude']) || empty($arr['longitude'])) return false ;
        else {
    ...
    private function buttonConstruct($arr=array()) {
        if (empty($arr)) return false ;
            ...
            if (!empty($arr['imageRatio'])) $template['template']['imageAspectRatio'] = $arr['imageRatio'] ;
            if (!empty($arr['imageSize'])) $template['template']['imageSize'] = $arr['imageSize'] ;
            if (!empty($arr['imageBackground'])) $template['template']['imageBackgroundColor'] = $arr['imageBackground'] ;
    ...
    private function carouselColumnConstruct($arr=array()) {
        if (empty($arr)) return false ;
            ...
            if (!empty($arr['imageBackground'])) $template['imageBackgroundColor'] = $arr['imageBackground'] ;
            if (!empty($arr['defaultAct']) && is_array($arr['defaultAct'])) {
    ...
    private function contentConstruct($arr=array()) {
        if (empty($arr)) {
            ...
            else if (count($contents) == 1) return $contents[0] ;
            else if (count($contents) > 1) return array_merge(array('type' => 'carousel'), array('contents' => $contents)) ;
        }
    }
    ##
        ...
        if (empty($arr)) {
            ...
            foreach ($arr as $k => $v) {
                if (!empty($v) && ($k == 'direction')) {
                    if (strtoupper($v) == 'L') $direction = 'ltr' ;
                    else if (strtoupper($v) == 'R') $direction = 'rtl' ;
                    else if (strtolower($v) == 'ltr') $direction = 'ltr' ;
                    else if (strtolower($v) == 'rtl') $direction = 'rtl' ;
                }
                else if (!empty($v) && ($k == 'header')) {  //Header block. Specify a box component.
                    //header style
                    if ($v['bgColor'])              $styles['header']['backgroundColor']    = $v['bgColor'] ;
                    unset($v['bgColor']) ;
                    
                    if ($v['backgroundColor'])      $styles['header']['backgroundColor']    = $v['backgroundColor'] ;
                    if (!empty($v['separator']))    $styles['header']['separator']          = true ;
                    if ($v['separatorColor'])       $styles['header']['separatorColor']     = $v['separatorColor'] ;
                    ##
                    ...
                    //hero style
                    if ($v['bgColor'])              $styles['hero']['backgroundColor']  = $v['bgColor'] ;
                    unset($v['bgColor']) ;
                    
                    if ($v['backgroundColor'])      $styles['hero']['backgroundColor']  = $v['backgroundColor'] ;
                    if (!empty($v['separator']))    $styles['hero']['separator']        = true ;
                    if ($v['separatorColor'])       $styles['hero']['separatorColor']   = $v['separatorColor'] ;
                    ##
                    ...
                    //body style
                    if ($v['bgColor'])              $styles['body']['backgroundColor']  = $v['bgColor'] ;
                    unset($v['bgColor']) ;
                    
                    if ($v['backgroundColor'])      $styles['body']['backgroundColor']  = $v['backgroundColor'] ;
                    if (!empty($v['separator']))    $styles['body']['separator']        = true ;
                    if ($v['separatorColor'])       $styles['body']['separatorColor']   = $v['separatorColor'] ;
                    ##
                    ...
                    //footer style
                    if ($v['bgColor'])              $styles['footer']['backgroundColor']    = $v['bgColor'] ;
                    unset($v['bgColor']) ;
                    
                    if ($v['backgroundColor'])      $styles['footer']['backgroundColor']    = $v['backgroundColor'] ;
                    if (!empty($v['separator']))    $styles['footer']['separator']          = true ;
                    if ($v['separatorColor'])       $styles['footer']['separatorColor']     = $v['separatorColor'] ;
                    ##
            ...
            if (!empty($direction)) $template = array_merge($template, array('direction'    => $direction)) ;
            if (!empty($header))    $template = array_merge($template, array('header'       => $header)) ;
            if (!empty($hero))      $template = array_merge($template, array('hero'         => $hero)) ;
            if (!empty($body))      $template = array_merge($template, array('body'         => $body)) ;
            if (!empty($footer))    $template = array_merge($template, array('footer'       => $footer)) ;
            if (!empty($styles))    $template = array_merge($template, array('styles'       => $styles)) ;
    ...
    private function createFlexBox($arr=array(), $recursive='') {
        ...
        foreach ($arr as $k => $v) {
            if (($k == 'type') && ($v == 'box')) {
                ...
                if (!empty($tmpBox)) $box = array_merge($box, array('contents' => $tmpBox)) ;
            }
            else if (($k == 'action') && (count($v) > 0)) {
                $box = array_merge($box, array('action' => $this->actionsBuild($v))) ;
            }
            else if ($k == 'btnTextColor') {
                if (strtoupper($v) == 'B') $box = array_merge($box, array('style' => 'secondary')) ;
                else if (strtoupper($v) == 'W') $box = array_merge($box, array('style' => 'primary')) ;
            }
            else if ($k == 'btnColor') {
                if (!empty($v)) $box = array_merge($box, array('color' => $v)) ;
            }
            else if ($k == 'layout') {
                if (strtoupper($v) == 'H') $box = array_merge($box, array('layout' => 'horizontal')) ;
                else if (strtoupper($v) == 'V') $box = array_merge($box, array('layout' => 'vertical')) ;
                else if (strtoupper($v) == 'B') $box = array_merge($box, array('layout' => 'baseline')) ;
            }
            else if ($k == 'wrap') {
                // if (strtoupper($v) == 'Y') $box = array_merge($box, array('wrap' => true)) ;
                if ($v) $box = array_merge($box, array('wrap' => true)) ;
            }
            else if ($k == 'align') {
                if (strtolower($v) == 'l') $box = array_merge($box, array('align' => 'start')) ;
                else if (strtolower($v) == 'c') $box = array_merge($box, array('align' => 'center')) ;
                else if (strtolower($v) == 'r') $box = array_merge($box, array('align' => 'end')) ;
            }
            else if ($k == 'txtColor') {
                if (preg_match("/^\#\w{6}$/isu", $v)) $box = array_merge($box, array('color' => $v)) ;
            }
            else if ($k == 'txtSize') {
                if (!empty($v)) $box = array_merge($box, array('size' => $v)) ;
            }
            else {
    ...
    public function push_reply($post_data, $tracking) {
        if (empty($this->reply_token)) 
            ...
            if ($json != '{}') $json =  $this->push($this->userId, $post_data, $tracking) ;
        }
